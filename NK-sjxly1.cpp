/*
在蓝色星球的2090年，一种表情包病毒已经席卷全球。每一个感染这种病毒的人只能说“表情包语言”。一句话表示为一个仅由小写英文字母组成的字符串 s。
它被归类为表情包语言的条件是满足以下所有要求：
字符串 s的长度恰好为 8。
位置1、3、5和7的字符是辅音（即它们不是`a`、`e`、`i`、`o`或`u`）。
位置2、4、6和8的字符是元音（`a`、`e`、`i`、`o`或`u`）。

例如，“bababoyi”和“bibilabu”是有效的表情包语言字符串。然而，“wodedaodun”不是（长度不是 8），“aiyogama”不是（第1个字符`a`不是辅音），而“ruheneng”也不是（第8个字符`g`不是元音）。
...
输入描述：
第一行包含一个整数𝑛（1 ≤ 𝑛 ≤ 10^5），表示要检查的人数。接下来的𝑛行，每行包含一个仅由小写英文字母组成的字符串𝑠，表示某人说的话。保证所有字符串𝑠的总长度不超过10^6。
...
int main()
{
    ...
    while (t--)
    {
        ...
        cout << (u ? "Suspected Virus" : "Well-Being") << "             " << s << endl;
    }
